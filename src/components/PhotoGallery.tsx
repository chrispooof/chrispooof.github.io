...
export default function PhotoGallery({ photos, photosId }: PhotoGalleryProps) {
  ...
  useEffect(() => {
    ...
    // window 'load' has already fired in a SPA by the time components mount,
    // so check image readiness directly instead
    ...
    if (unloaded.length === 0) {
      ...
      unloaded.forEach((img) => img.addEventListener('load', onImageLoad))
    }
  ...
  return (
    <div ref={divRef} className="flex flex-row flex-nowrap overflow-x-auto">
      {photos.map((photo) => (
        <img key={photo} src={photo} className="block w-auto h-auto max-h-[20vw] m-0.5" alt="" />
      ))}
    </div>
  )
}
